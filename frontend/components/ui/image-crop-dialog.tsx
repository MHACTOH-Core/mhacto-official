...
import { ZoomIn, RotateCw, Crop, Sparkles, Loader2 } from "lucide-react"
import { Switch } from "@/components/ui/switch"

// ─── Inline image enhancement (sharpen kernel) ────────────────────
// Inlined here to avoid module-resolution caching issues with the TS server.
// Applies a 3×3 unsharp-mask convolution followed by a subtle contrast bump.
function enhanceImage(
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      const top = ((y-1)*width+x)*4, bot = ((y+1)*width+x)*4
      const lft = (y*width+(x-1))*4, rgt = (y*width+(x+1))*4
      for (let c = 0; c < 3; c++) {
        const center = src[idx+c]
        const neighbours = src[top+c]+src[bot+c]+src[lft+c]+src[rgt+c]
        output[idx+c] = center + k*(4*center - neighbours)
      }
...
export interface ImageCropDialogProps {
  ...
  imageSrc: string
  /** Called with the final Blob (cropped + optionally enhanced). */
...
export function ImageCropDialog({
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      const blob = await getCroppedBlob(imageSrc, croppedArea, enhance)
      onCropComplete(blob)
      onOpenChange(false)
    } finally {
      setSaving(false)
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-lg">
        ...
        {/* Crop area */}
        <div className="relative mx-auto h-80 w-full overflow-hidden rounded-xl bg-muted">
          <Cropper
            image={imageSrc}
            crop={crop}
            zoom={zoom}
