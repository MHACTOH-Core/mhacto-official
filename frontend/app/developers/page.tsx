  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#0a0e1a]">
      ...
      <section className="relative min-h-[85vh] flex items-center justify-center overflow-hidden">
        ...
        {/* Vignette */}
        <div className="absolute inset-0 pointer-events-none bg-[radial-gradient(ellipse_at_center,transparent_50%,rgba(10,14,26,0.6))]" aria-hidden />
      ...
      {/* ══════════ ABOUT ══════════ */}
      <section className="relative bg-[#0d1225]">
      ...
      <section className="relative bg-[#0a0e1a]">
        <div className="absolute inset-0 pointer-events-none" aria-hidden>
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[600px] rounded-full bg-blue-500/[0.03] blur-[100px]" />
          <div className="absolute bottom-0 right-1/4 w-[400px] h-[400px] rounded-full bg-violet-500/[0.02] blur-[80px]" />
        </div>

        <div className="relative z-10 mx-auto max-w-5xl px-6 py-20 sm:py-28">
          <div className="reveal-on-scroll reveal-scale text-center mb-16">
            <p className="text-xs font-semibold uppercase tracking-[0.25em] text-cyan-400 mb-3">Our People</p>
            <h2 className="text-3xl sm:text-4xl font-bold text-white">The Developers</h2>
            <div className="mt-5 mx-auto w-16 h-px bg-gradient-to-r from-transparent via-cyan-500/50 to-transparent" />
          </div>

          {/* Lead card — 3D tilt */}
          <div className="reveal-on-scroll reveal-blur mb-10">
            {(() => {
              const lead = team[0]
              ...
              return (
                <div
                  className="dev-tilt-card group relative rounded-2xl border border-white/[0.06] bg-white/[0.02] backdrop-blur-sm overflow-hidden mx-auto max-w-3xl hover:border-white/[0.12] hover:bg-white/[0.04] hover:shadow-2xl hover:shadow-amber-500/[0.05]"
                  onMouseMove={handleTilt}
                  onMouseLeave={resetTilt}
                >
                  <div className={`h-px bg-gradient-to-r ${lead.gradient} opacity-40 group-hover:opacity-100 transition-opacity duration-500`} />
                  <div className="p-8 sm:p-10 flex flex-col sm:flex-row items-center gap-6 text-center sm:text-left">
                    <div className="relative">
                      <div className={`absolute inset-0 rounded-full bg-gradient-to-br ${lead.gradient} opacity-0 group-hover:opacity-25 blur-xl transition-opacity duration-700`} />
                      <div className={`relative flex h-20 w-20 items-center justify-center rounded-full ${lead.avatarBg} text-white text-xl font-bold flex-shrink-0 shadow-lg ring-2 ring-white/10 group-hover:ring-white/25 transition-all duration-500 group-hover:scale-110`}>
                        {initials}
                      </div>
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center justify-center sm:justify-start gap-2 mb-4">
                        <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold ${lead.roleBg} ${lead.roleText} border border-current/10`}>
                    ...
                    {lead.linkedin && (
                      <a
                        ...
                        className="dev-magnetic inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-white/5 border border-white/10 text-white text-sm font-medium hover:bg-[#0A66C2] hover:border-[#0A66C2]/50 hover:shadow-lg hover:shadow-blue-500/20 flex-shrink-0"
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            {team.slice(1).map((member, idx) => {
              ...
              const delay = `delay-${(idx + 1) * 100}`
              const direction = idx % 2 === 0 ? "reveal-left" : "reveal-right"

              return (
                <div key={member.name} className={`reveal-on-scroll ${direction} ${delay}`}>
                  <div
                    className={`dev-tilt-card group relative rounded-2xl border border-white/[0.06] bg-white/[0.02] backdrop-blur-sm overflow-hidden hover:border-white/[0.12] hover:bg-white/[0.04] hover:shadow-2xl ${member.glowColor}`}
                    onMouseMove={handleTilt}
                    onMouseLeave={resetTilt}
                  >
                    <div className={`h-px bg-gradient-to-r ${member.gradient} opacity-0 group-hover:opacity-100 transition-opacity duration-500`} />
                    <div className="p-6 sm:p-7">
                      <div className="flex items-start gap-4 mb-4">
                        <div className="relative flex-shrink-0">
                          <div className={`absolute inset-0 rounded-full bg-gradient-to-br ${member.gradient} opacity-0 group-hover:opacity-25 blur-lg transition-opacity duration-700`} />
                          <div className={`relative flex h-14 w-14 items-center justify-center rounded-full ${member.avatarBg} text-white text-sm font-bold shadow-lg ring-2 ring-white/10 group-hover:ring-white/25 transition-all duration-500 group-hover:scale-110`}>
                            {initials}
                          </div>
                        </div>
                        <div className="min-w-0">
                          <h3 className="text-[15px] font-bold text-white leading-snug group-hover:text-white/90 transition-colors">{member.name}</h3>
                          <span className={`inline-flex items-center gap-1.5 mt-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold ${member.roleBg} ${member.roleText}`}>
                      ...
                      {member.linkedin && (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="dev-magnetic inline-flex items-center gap-1.5 px-4 py-2 rounded-full bg-white/5 border border-white/10 text-slate-300 text-xs font-medium hover:bg-[#0A66C2] hover:border-[#0A66C2]/50 hover:text-white hover:shadow-lg hover:shadow-blue-500/20"
                          onMouseMove={handleMagnetic}
                          onMouseLeave={resetMagnetic}
                        >
