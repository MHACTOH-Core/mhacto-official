<?php
/**
 * HomeContent Model — Optimized schema.
 *
 * REMOVED sections (no longer separate tables):
 *   - Hero Slides  → single hero in site_settings (use Settings model)
 *   - Culinary     → auto-pulled from CMS label 'local-cuisine'
 *
 * MERGED sections:
...
class HomeContent
{
    ...
    /**
     * Shared SELECT for featured_content with CMS data.
     */
    ...
    /**
     * Get the first thumbnail image for a content row.
     */
    ...
    {
        // If setting as active, deactivate other spotlights first
    ...
    // ── HERO SETTINGS (convenience — delegates to site_settings) ──

    public function getHeroSettings()
    {
        $stmt = $this->conn->prepare("
            SELECT settings_id AS settingId,
                   hero_subtitle AS subtitle, hero_title AS title,
                   hero_highlight AS highlight, hero_description AS description,
                   hero_video_url AS videoUrl, hero_fallback_img AS fallbackImage,
                   hero_cta_text AS ctaText, hero_cta_link AS ctaLink,
    ...
    public function updateHeroSettings($data)
    {
        ...
        $check = $this->conn->query("SELECT settings_id FROM site_settings ORDER BY settings_id ASC LIMIT 1")->fetch(PDO::FETCH_ASSOC);

        if (!$check) {
            // No settings row yet — insert with hero defaults
            $sql = "INSERT INTO site_settings (hero_subtitle, hero_title, hero_highlight, hero_description, hero_video_url, hero_fallback_img, hero_cta_text, hero_cta_link)
                    VALUES (:subtitle, :title, :highlight, :description, :videoUrl, :fallbackImage, :ctaText, :ctaLink)";
            $stmt = $this->conn->prepare($sql);
            return $stmt->execute([
                ...
                ':ctaLink'       => $data['ctaLink'] ?? '/destinations',
            ]);
        }

        $sql = "UPDATE site_settings SET " . implode(', ', $fields) . " WHERE settings_id = :id";
        $params[':id'] = $check['settings_id'];
        $stmt = $this->conn->prepare($sql);
        return $stmt->execute($params);
    }

    // ─────────────────────────────────────────────────────────────────
    // MILESTONES (History Timeline) — unchanged
    // ─────────────────────────────────────────────────────────────────
    ...
    private function formatFeatured(array $row): array
    {
        ...
        return [
            ...
            'category'      => $row['place_category'] ?? $row['category_name'] ?? null,
            'date'          => $row['news_date'] ?? null,
